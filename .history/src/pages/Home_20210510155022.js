...
function Home() {
  ...
  for (var x = 0; sortedArray.length > x; x++) {
    dropDownList.push(localStorage.getItem(sortedArray[x]));
  }

  var arrayOfValues = dropDownList.slice(-5);
          ...
          <Form>
                    ...
                    .map((value, i) => {
                      return (
                        <div
                          className="option"
                          key={i}
                          onClick={(event) => setValue(event.target.innerText)}
                        >
                          <span>{value}</span>
                        </div>
                      );
            ...
            <ButtonList>
              <Button onClick={handleSubmit}>
                Search2! <BsFillImageFill style={{ padding: "0 5px" }} />
              </Button>
              <LogoutButton />
            </ButtonList>
          </Form>
